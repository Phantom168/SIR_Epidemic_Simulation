...
int max_time=300; //Max time the simulation will run for.

int currDay=0; //Variable to store the current day.

//int adj_matrix[MAX_VERTICES][MAX_VERTICES]; //Create Adjacency matrix to create the graph
int **adj_matrix;

void Insert(lNode** head,vertex* v); //Prototype
void print_list(lNode* head); //Prototype
void Shift(lNode** from_head,lNode** to_head,vertex* v); //Prototype
void Fast_SIR(vertex *G,float tao,float gamma,int* initial_infecteds,int g_size,int **adj_matrix,int t_max,int inf_size); //Prototype
void Swap(event** a,event** b); //Prototype
void min_heapify(event **q,int i,int *size); //Prototype
event extract(event **q,int* size); //Prototype
void InsertQ(event **q,event *e,int *size); //Prototype
void PrintEventQ(event** pq,int size,int day); //Prototype
int getDays(float prob); //Prototype
void process_trans_SIR(vertex *G,vertex* target,float tao,float gamma,int time,int g_size,int **adj_matrix); //Prototype
void process_rec_SIR(vertex* v,int time); //Prototype
int minimum(int a,int b,int c); //Prototype
void printAll(); //Prototype
...
int main()
{
  ...
  adj_matrix = (int**)malloc(MAX_VERTICES*sizeof(int*));
  for(int i=0;i<MAX_VERTICES;i++){
    adj_matrix[i]= (int*)malloc(MAX_VERTICES*sizeof(int));
  }
  priorityQ = (event**)malloc(MAX_VERTICES*sizeof(event*)); //Allocate Memory to the priority queue double pointer.
  float tao=0.5; //Probability of getting infected.
  float gamma=0.2; //Probability of getting recovered.
  ...
  srand(time(NULL)); //Seed Random function with time to increase randomness.
  int noOfVert = rand()%MAX_VERTICES; //Generate random no that gives number of vertices in graph.

  int maxNoOfEdges = rand()%MAX_EDGES; //Generate random no that gives max number of edges in graph.
...
void process_trans_SIR(vertex *G,vertex* target,float tao,float gamma,int time,int g_size,int **adj_matrix)
{
  Shift(&s_list,&i_list,target); //Shift the node target from susceptible to infected.
  s_size--;
  i_size++;
...
void Fast_SIR(vertex *G,float tao,float gamma,int* initial_infecteds,int g_size,int **adj_matrix,int t_max,int inf_size)
{
